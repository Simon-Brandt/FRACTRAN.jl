...
Since the FRACTRAN algorithm is rather slow, especially PRIMEGAME, FRACTRAN.jl also includes two module-level caches, [`FRACTRAN.factorizations`](@ref) and [`FRACTRAN.primes`](@ref).  These caches may be used for accelerating repeated [`fractran`](@ref) calls.  To this end, [`factorize`](@ref), [`fractran`](@ref), and all example programs have a second form with exclamation mark (like [`factorize!`](@ref), [`fractran!`](@ref) *etc.*), that take caches as additional arguments for in-place mutation.  You may either use the provided module-level caches or pass your own objects to them, depending on the intended persistence of the caches.
...
!!! note
    In order not to clutter your namespace upon `using FRACTRAN`, the caches and example programs are only declared as `public`, but not `export`ed.  You can access them by prefixing them with the module name, i.e., as [`FRACTRAN.add`](@ref) *etc.*
